"""
Given an array of integers a, your task is to find how many of its contiguous subarrays of length m 
...
such that a subarray [a[i], a[i + 1], ..., a[i + m - 1]] contains at least one pair (a[s], a[t]), where:
...
Subarray a[0..3] = [2, 4, 7, 5] doesn't contain any pair of integers with a sum of k = 10. Note that although 
the pair (a[3], a[3]) has the sum 5 + 5 = 10, it doesn't fit the requirement s ≠ t.
...
def solution(a, m, k):
    position = 0
    count = 0
    while position <= len(a)-m:
        substring = a[position:position+m]
        position+=1
        
        while substring:
            num = substring.pop()
            print(f"Checking if {k-num} in substring {substring}")
            if k-num in substring:
                count += 1
                break
    return count
